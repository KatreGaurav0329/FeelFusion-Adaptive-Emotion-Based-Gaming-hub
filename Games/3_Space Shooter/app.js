...
// Global Declarations //
//~~~~~~~~~~~~~~~~~~~~~//
// DOM Elements for both emotion detection 
...
const emotionSpeeds = {
  neutral:   { alienSpeed: 3, missileCooldown: 10 },
  happy:     { alienSpeed: 3.2, missileCooldown: 9 },
  sad:       { alienSpeed: 2.8, missileCooldown: 11 },
  angry:     { alienSpeed: 3.5, missileCooldown: 8 },
  surprised: { alienSpeed: 3.3, missileCooldown: 9 },
...
updateMissiles = () => {
	...
	if (keyState[' '] && ship.alive && ship.missiles > 0 && missilePool.cooldown == 0 && missilePool.ready.length > 0) {
		...
		missilePool.cooldown = Math.round(currentMissileCooldown);
		ship.missiles -= 1;
	}
...
updateState = () => {
	// Smooth interpolation (the 0.07 factor controls speed of transition)
	currentAlienSpeed += (targetAlienSpeed - currentAlienSpeed) * 0.07;
	currentMissileCooldown += (targetMissileCooldown - currentMissileCooldown) * 0.07;
	currentAlienSpeed = Math.max(2, Math.min(currentAlienSpeed, 6));
	currentMissileCooldown = Math.max(5, Math.min(currentMissileCooldown, 20));

	displayDifficulty(lastEmotion);
...
function updateGameDifficulty(emotion) {
  ...
  targetAlienSpeed = settings.alienSpeed;
  targetMissileCooldown = settings.missileCooldown;
}
...
function displayDifficulty(emotion) {
  ...
  difficultyDiv.innerHTML = `
    ...
    <strong>Alien Speed:</strong> ${currentAlienSpeed.toFixed(2)}<br/>
    <strong>Missile Cooldown:</strong> ${currentMissileCooldown.toFixed(0)} frames
  `;
}
...
video.addEventListener('play', () => {
  ...
  const detectionInterval = setInterval(async () => {
    ...
    if (detections.length > 0) {
	  ...
	  // Only update target difficulty if emotion CHANGED
